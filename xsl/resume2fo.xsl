...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/XSL/Format"
    ...
    <root xmlns="http://www.w3.org/1999/XSL/Format">
      ...
      <page-sequence master-reference="resume">
        <static-content flow-name="xsl-region-before">
          <block font-family="Helvetica" font-size="12pt" text-align="right">
            <xsl:apply-templates select="resume:ContactInfo" mode="header" />
          </block>
        </static-content>
        <static-content flow-name="xsl-region-after">
          <block font-family="Helvetica" font-size="9pt">
            <xsl:apply-templates select="resume:ContactInfo" mode="footer" />
          </block>
        </static-content>
        <flow flow-name="xsl-region-body">
          <block font-family="Helvetica" font-size="11pt">
  ...
  <xsl:template match="resume:Achievements">
    <block margin-bottom="25pt">
      ...
      <block font-size="90%" line-height="150%" margin-bottom="10pt" margin-right="20pt">
        <list-block>
  ...
  <xsl:template match="resume:Associations">
    <block margin-bottom="25pt">
      ...
      <xsl:for-each select="resume:Association">
        <block font-size="90%" margin-bottom="10pt" margin-right="20pt">
  ...
  <xsl:template match="resume:EducationHistory">
    <block margin-bottom="25pt">
      ...
      <xsl:for-each select="resume:SchoolOrInstitution">
        <block font-size="90%" keep-with-previous="always" margin-bottom="10pt" margin-right="20pt">
          <xsl:value-of select="resume:Degree/resume:DegreeName" />
          <xsl:text>,&#x20;</xsl:text>
          <xsl:value-of select="resume:SchoolName" />
          <xsl:text>,&#x20;</xsl:text>
          <xsl:value-of select="resume:Degree/resume:DegreeMeasure/resume:EducationalMeasure/resume:MeasureSystem" />
          <xsl:text>&#x20;</xsl:text>
          <xsl:value-of select="resume:Degree/resume:DegreeMeasure/resume:EducationalMeasure/resume:MeasureValue/resume:NumericValue" />
              ...
              <list-item>
                ...
                <list-item-body>
                  <block margin-left="10pt">
                    ...
                    <xsl:value-of select="resume:Degree/resume:DegreeMinor/resume:Name" />
                  </block>
                </list-item-body>
              </list-item>
              <xsl:if test="resume:Degree/resume:Comments">
                <xsl:for-each select="resume:Degree/resume:Comments">
                  <list-item>
                    <list-item-label>
                      <block>&#x2022;</block>
                    </list-item-label>
  ...
  <xsl:template match="resume:EmploymentHistory">
    <block margin-bottom="25pt">
      <xsl:call-template name="getTitleBlock">
        <xsl:with-param name="title" select="string('Work Experience')" />
      </xsl:call-template>
      <xsl:for-each select="resume:EmployerOrg">
        <xsl:if test="position() &lt;= 3">
          <block font-size="90%" margin-bottom="10pt" margin-right="10pt">
            <block font-weight="bold" margin-bottom="5pt">
              <table table-layout="fixed" width="100%">
                <table-column column-width="75%" />
                <table-column column-width="25%" />
  ...
  <xsl:template match="resume:ExecutiveSummary">
    <block margin-bottom="25pt">
      ...
      <block font-size="90%" margin-bottom="10pt" margin-right="20pt" text-align="justify">
        <xsl:value-of select="." />
      </block>
    </block>
  </xsl:template>
  ...
  <xsl:template match="resume:Qualifications">
    <block margin-bottom="25pt">
      <xsl:call-template name="getTitleBlock">
        <xsl:with-param name="title" select="string('Volunteer and Club Experience')" />
      </xsl:call-template>
      <block font-size="90%" line-height="150%" margin-bottom="10pt" margin-right="20pt">
        <list-block>
          <xsl:for-each select="resume:Competency">
  ...
  <xsl:template match="resume:ResumeAdditionalItems">
    <block margin-bottom="25pt">
      <xsl:call-template name="getTitleBlock">
        <xsl:with-param name="title" select="string('Personal Interests')" />
      </xsl:call-template>
      <block font-size="90%" line-height="150%" margin-bottom="10pt" margin-right="20pt"
  ...
  <xsl:template name="getTitleBlock">
    ...
    <block-container border-bottom-style="dashed" border-width=".5pt" margin-bottom="10pt">
      <block text-align="right" text-transform="uppercase">
